...
app.post("/chat", async (req, res) => {
  try {
    ...
    // 1. Log the input to ensure the server received it
...
`
    ...
    // 3. Log the full response to debug if "undefined" happens again
    console.log("OpenAI Response:", JSON.stringify(response, null, 2));

    // 4. Safe check: Ensure the response structure is valid before accessing .content
    if (response.choices && response.choices.length > 0 && response.choices[0].message) {
        const botReply = response.choices[0].message.content;
        res.json({ reply: botReply });
    } else {
        console.error("Error: API returned an empty choice list.");
        res.status(500).json({ error: "No response generated by AI." });
    }

  } catch (error) {
    // 5. Log the actual error for debugging
